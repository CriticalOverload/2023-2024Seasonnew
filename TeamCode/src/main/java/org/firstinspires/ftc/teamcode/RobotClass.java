...
public class RobotClass {
    ...
    private DistanceSensor distSenseBack;
    private DistanceSensor distSenseFront;
    ...
    public RobotClass(DcMotor motorFrontRight, DcMotor motorFrontLeft, DcMotor motorBackRight, DcMotor motorBackLeft,
        ...
        this.bucket = bucket;
        this.duck = duck;
        this.distSenseBack = distSenseBack;
        this.distSenseFront = distSenseFront;
        this.imu = imu;
        this.opMode = opMode;
...
//
     ...
     */
    public void driveToWallBack(double power, double dist) throws InterruptedException{
        telemetry.addData("reached","drivetowallback");
        telemetry.addData("distance reading", distSenseBack.getDistance(DistanceUnit.CM));
        telemetry.update();

        resetAngle();
        //calculate powers needed using direction
        double leftPower = -1*power;
        double rightPower = -1*power;
        double correction = getCorrection();
        correctedTankStrafe(leftPower, rightPower, correction);
        while(distSenseBack.getDistance(DistanceUnit.CM) > dist && opMode.opModeIsActive()){
            correction = getCorrection();
            correctedTankStrafe(leftPower, rightPower, correction);
        //    telemetry.addData("correction", correction);
        //     telemetry.addData("leftPower:", leftPower);
            telemetry.addData("target distance:", 35);
            telemetry.addData("distance reading", distSenseBack.getDistance(DistanceUnit.CM));
            telemetry.update();
//            Thread.sleep(250);//todo REMOVE THIS
        }
        completeStop();
        Thread.sleep(250);
        resetAngle();
     ...
     * @throws InterruptedException
     */
    public void driveToWallFront(double power, double dist) throws InterruptedException{
        resetAngle();

        while(distSenseFront.getDistance(DistanceUnit.INCH) > dist && opMode.opModeIsActive()){
            double correction = getCorrection();
            correctedTankStrafe(power, power, correction);
            //    telemetry.addData("correction", correction);
            //     telemetry.addData("leftPower:", leftPower);
            //     telemetry.addData("rightPower:", rightPower);
            telemetry.addData("distance reading", distSenseFront.getDistance(DistanceUnit.INCH));
            telemetry.update();
        }
        completeStop();
        Thread.sleep(250);
...
//            telemetry.addData("circumference", DRIVE_WHEEL_CIRCUMFERENCE);
    ...
    /**
     * For Autonomous Red
     * After dumping in alliance hub, drive to warehouse
     assuming that we're between alliance wall and the hub, vs in a different position!!!
     * @param goOverBarrier if true, robot will run over barrier. False, will go along side
     * @param power
     * */
    public void goToWarehouse_RedW(boolean goOverBarrier, double power) throws InterruptedException{
        if(!goOverBarrier) {
            //go to warehouse
            telemetry.addData("hey yo","go here...");
            telemetry.update();
            gyroTurn(-90, 0.5);
            gyroStrafeEncoder(0.5, 180, 34);//change distance
...
//            gyroStrafeEncoder(0.5, -90, 56);//???
            ...
            telemetry.addData("moving","slides");
            telemetry.update();
            rTPMoveSlides(1,0.5);
        }
    ...
    public void goToWarehouse_Blue(boolean goOverBarrier, double power) throws InterruptedException{
        if(!goOverBarrier) {
            ...
            telemetry.update();
            gyroTurn(90, 0.5);
            gyroStrafeEncoder(0.5, 0, 33);//change distance
        }
        else{ //option 2: there's a robot in the way, and we need to instead go over the bumps...
            gyroTurn(90, 0.5);
            rTPMoveSlides(1,0.5);
        }
        driveToWallBack(power,43);
        if(goOverBarrier)
            rTPMoveSlides(0, 0.5);
    }
...
//        gyroTurn(90,0.5);
        driveToWallFront(0.5,10);
        // driveToWall(0.5);//other option... idk
//        gyroStrafeEncoder(0.5,180,8);
        gyroStrafeEncoder(0.5,180,10.75);
    }
...
//        robot.driveToWall(0.5);//other option
        driveToWallFront(0.5,12);
        gyroStrafeEncoder(0.5,0,20);
    }
    ...
    //uses distance sensor to drive to carousel
    public void goToCarousel(double power) throws InterruptedException{
        resetAngle();

        while(distSenseFront.getDistance(DistanceUnit.INCH) > 25 && opMode.opModeIsActive()){
            double correction = getCorrection();
            correctedTankStrafe(power, power, correction);
            //    telemetry.addData("correction", correction);
            //     telemetry.addData("leftPower:", leftPower);
        ...
        while(distSenseFront.getDistance(DistanceUnit.INCH) > 12 && opMode.opModeIsActive()){
            double correction = getCorrection();
            correctedTankStrafe(power/2, power/2, correction);
            //    telemetry.addData("correction", correction);
            //     telemetry.addData("leftPower:", leftPower);
            //     telemetry.addData("rightPower:", rightPower);
            telemetry.addData("distance reading", distSenseFront.getDistance(DistanceUnit.INCH));
        ...
        while(distSenseFront.getDistance(DistanceUnit.INCH) > 11.75 && opMode.opModeIsActive()){
            double correction = getCorrection();
            correctedTankStrafe(power/4, power/4, correction);
            //    telemetry.addData("correction", correction);
            //     telemetry.addData("leftPower:", leftPower);
            //     telemetry.addData("rightPower:", rightPower);
            telemetry.addData("distance reading", distSenseFront.getDistance(DistanceUnit.INCH));
